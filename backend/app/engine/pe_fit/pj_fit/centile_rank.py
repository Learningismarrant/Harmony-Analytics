# engine/recruitment/pe_fit/pj_fit/centile_rank.py
"""
Rang Centile Dynamique (Π_pool) — module pe_fit/pj_fit.

Copie fonctionnelle de DNRE/centile_rank.py.
Aucune modification de logique — les imports sont autonomes (ne dépend plus de DNRE).
...
Formule de Tukey :
    ...
    N     = taille totale du pool

Le centile est dynamique : il correspond au pool des candidats ayant
postulé à CETTE campagne, recalculé à chaque nouveau candidat.
...
class CentileResult:
    """
    Résultat du rang centile pour un candidat.
...
def compute(
    ...
    Args:
        candidate_score : score du candidat (0-100)
        pool_scores     : liste des scores de TOUS les candidats du pool
    ...
    if n < MIN_POOL_SIZE_VALID:
        ...
        confidence = "MEDIUM"
        flags.append(
            f"SMALL_POOL [{competency_key}]: N={n} < {MIN_POOL_SIZE_RELIABLE}, "
            f"centile approximatif"
        )
    else:
        confidence = "HIGH"
...
def compute_batch(
    ...
    Args:
        candidates_scores : {crew_profile_id: score}
        competency_key    : pour le logging
